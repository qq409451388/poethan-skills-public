...
仅在用户明确说“进入代码检查模式”、“开启代码检查模式”或 `/code-inspector start` 后执行本流程；退出方式见 `references/activation.yaml`。

安装后的 Skill 会生成当前平台可用的逻辑身份、角色能力和固定工具入口。一个实际 Agent 可以承担多个角色，但每个角色必须使用不同 alias；同一角色有多个 alias 时，安装配置必须指定唯一默认身份。只使用当前任务对应身份的入口，不直接访问 SQLite 或执行 SQL。
