...
Poethan Sentinel 是一套可扩展的远程服务器诊断工具。客户端负责编排服务器、诊断工具、执行结果、AI 分析和报告；诊断逻辑可以来自本机脚本、服务器已有脚本或带清单与报告模板的完整插件。

诊断工具库统一管理三类来源：本机脚本会按版本同步到目标服务器，服务器脚本只登记并调用远端绝对路径，插件则继续使用 `plugin.yaml`、文件摘要和数字签名完成可信校验。
