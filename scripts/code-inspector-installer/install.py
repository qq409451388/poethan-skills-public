...
def generated_skill_text(platform: str, identities: list[dict[str, Any]], target: Path) -> str:
    ...
    routing = (
        ...
        "`<fixed_tool> issue-context-get --issue-key <issue_key>`。`issue-context-get` 使用 `--issue-key`，不使用 `--issue-id`。"
        "以返回的 pending_action、permitted_actions、exception_actions 和资源 id 为 Working Set；只有需要正文时才 "
        ...
        "FastMode 的 PASS/FAIL 只使用 Inspector 专用的 `fast-review-record --issue-key <issue_key> "
        "--decision pass|fail --content <result_summary> [--evidence <evidence_summary>]` 写入；普通 `metadata` "
        "只记录事实，不能启用 FastMode 或改变权限和控制流。"
        "FastMode 必须读取 `references/fastmode.md`，首次读取固定执行 "
        "`<fixed_tool> issue-context-get --issue-key <issue_key>`，使用 `--issue-key`，不要使用 `--issue-id`。\n\n"
...
def generated_codex_rules(identities: list[dict[str, Any]], skill_target: Path) -> str:
    ...
    for item in identities:
        wrapper = str(skill_target / "tools" / f"review-db-{item['alias']}.py")
