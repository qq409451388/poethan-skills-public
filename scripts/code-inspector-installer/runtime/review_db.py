...
def normalize_issue_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized.update({
        ...
        # difficulty 是 Inspector 输出的抽象困难程度；Router 只依赖它，不感知模型名称。
        "difficulty": parse_difficulty(normalized.get("difficulty")),
        "difficulty_reason": issue_json_value(normalized, "difficulty_reason", []),
    })
    if not isinstance(normalized["difficulty_reason"], list):
        raise ValueError("difficulty_reason 必须是 JSON 数组")
...
def issue_query(conn: sqlite3.Connection, args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for r in conn.execute(sql, params).fetchall():
        ...
        for key in (
            ...
            "difficulty_reason_json", "recommended_executors_json", "assignment_json"
        ):
            item[key] = loads(
                item[key],
...
def apply_difficulty_update(
    conn: sqlite3.Connection, args: argparse.Namespace, issue_key: str,
    difficulty: int | None, reason: list[Any] | None,
) -> dict[str, Any]:
    """追加/修正 Inspector 的抽象困难程度，并重算推荐执行配置。

    Router 未启用时推荐列表为空；本函数不改变 Issue 状态机。
    """
    row = conn.execute("SELECT id FROM review_issue WHERE issue_key = ?", (issue_key,)).fetchone()
    if not row:
        raise KeyError(f"问题不存在: {issue_key}")
...
def issue_set_difficulty(args: argparse.Namespace) -> None:
    require_agent(args.agent)
    if args.agent not in {"inspector", "human"}:
        raise PermissionError("只有 inspector 或 human 可以修改问题困难程度")
    difficulty = parse_difficulty(args.difficulty)
    reason = json.loads(args.difficulty_reason) if args.difficulty_reason is not None else None
    with connect() as conn:
        result = apply_difficulty_update(conn, args, args.issue_key, difficulty, reason)
    print_json({"issue_key": args.issue_key, **result})
...
def apply_assignment_update(
    ...
    conn.execute(
        "UPDATE review_issue SET assignment_json = ?, updated_at = CURRENT_TIMESTAMP "
        "WHERE issue_key = ?",
        (dumps(assignment), issue_key),
    )
    audit(conn, actor_id(args), "issue.set-assignment", "review_issue", issue_key, True,
          f"profile={profile_id or 'cleared'}")
    return assignment


def issue_set_assignment(args: argparse.Namespace) -> None:
    require_agent(args.agent)
    if args.agent not in {"inspector", "human"}:
        raise PermissionError("只有 inspector 或 human 可以指定执行配置")
    with connect() as conn:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--local-terms", default="{}", help='JSON 对象；本地术语表，如 {"下单": "OrderService.place"}')
    ...
    p.add_argument("--trigger-conditions", default="[]", help='JSON 数组；触发条件，如 ["库存为 0 时仍可提交订单"]')
    p.add_argument("--potential-impact", default="[]", help='JSON 数组；潜在影响，如 ["超卖"]')
    p.add_argument("--impact-scope", default="[]", help='JSON 数组；影响范围，如 ["order-service"]')
    p.add_argument("--rationale")
    p.add_argument("--evidence", default="[]", help='JSON 数组；每项优先 file_path 加 symbol 或 code_excerpt，如 [{"file_path":"Order.java","symbol":"place"}]')
    p.add_argument("--estimated-change", default="{}", help='JSON 对象；预计改动，如 {"files":2,"tables":1}')
    p.add_argument("--difficulty", help="Inspector 输出的抽象困难程度，正整数；越高要求越高")
    p.add_argument("--difficulty-reason", default="[]", help='JSON 数组；解释困难程度的要点，如 ["涉及跨模块修改"]')
    ...
    p.add_argument("--updates", required=True, help='JSON 数组；每项含 issue_key 和 status，如 [{"issue_key":"RI-1","status":"ON_HOLD","content":"原因"}]')
    ...
    p.add_argument("--code-reference", default="[]", help='JSON 数组；代码定位，如 [{"file_path":"Order.java","symbol":"place"}]')
    p.add_argument("--metadata", default="{}", help="JSON 对象；只记录业务事实，不改变权限或控制流，如 {}")
    ...
    p.add_argument("--content", required=True)
    p.add_argument("--scope-changes", help='JSON 数组；Developer 必填，无范围外变化传 []；每项 {"id":"SC-x","summary":"白话描述","impacts":["new_persistence"]}')
    p.add_argument("--code-reference", default="[]", help='JSON 数组；代码定位，如 [{"file_path":"Order.java","symbol":"place"}]')
    p.add_argument("--metadata", default="{}", help="JSON 对象；只记录业务事实，不改变权限或控制流，如 {}")
    ...
    p.add_argument("--confirmation-ids", help="JSON 数组；design-choice-record 返回的 decision id，如 [1,2]")
    p.add_argument("--stages", help='JSON 数组；staged 批准时传入，每项含正整数 stage_no、title、objective、acceptance_criteria，如 [{"stage_no":1,"title":"...","objective":"...","acceptance_criteria":["..."]}]；已有旧计划时省略')
    p.add_argument("--content", required=True)
    ...
    p.add_argument("--summary", required=True)
    p.add_argument("--impacts", required=True, help='JSON 数组；影响类型取值 new_persistence/data_migration/external_behavior/new_dependency/scope_expansion/multiple_solutions，如 ["data_migration"]')
    p.add_argument("--change-ids", required=True, help="JSON 数组；本回答覆盖的 scope change id，如 [\"SC-1\"]")
    ...
    p.add_argument("--issue-key", required=True)
    p.add_argument("--stage-no", required=True, type=int)
    p.add_argument("--change-scope", required=True, help="JSON 对象或数组；预计影响的模块、文件或类，如 [\"order-service\",\"Order.java\"]")
    p.add_argument("--change-reason", required=True)
    p.add_argument("--protected-behaviors", default="[]", help='JSON 数组；必须保护的历史行为契约，如 ["库存为 0 时禁止提交订单"]')
    ...
    p.add_argument("--code-reference", default="[]", help='JSON 数组；本次修改的代码定位，如 [{"file_path":"Order.java","symbol":"place"}]')
    p.add_argument("--test-evidence", default="[]", help='JSON 数组；测试证据，如 [{"type":"unit","name":"testPlace","result":"PASS"}]')
    p.add_argument("--resolved-findings", default="[]", help="JSON 数组；本轮已修复的 BLOCKER/MUST finding id，如 [\"F-1\"]")
    p.add_argument("--metadata", default="{}", help="JSON 对象；只记录业务事实，不改变权限或控制流，如 {}")
    ...
    p.add_argument("--reason", required=True)
    p.add_argument("--question", required=True)
    p.add_argument("--options", default="[]", help='JSON 数组；可选方案，如 ["方案A：保留现库表并双写","方案B：直接迁移"]')
    p.add_argument("--evidence", default="[]", help='JSON 数组；已验证事实，如 ["订单表存在无校验写入路径"]')
    ...
    p.add_argument("--updates", required=True, help='JSON 数组；每项含 issue_key 加要更新的评级字段，如 [{"issue_key":"RI-1","severity":"high"}]')
    ...
    p.add_argument("--expected-outcome")
    p.add_argument("--technical-note")
    p.add_argument("--local-terms", help='更新本地术语表时必传 JSON 对象，如 {"下单": "OrderService.place"}；不更新则省略')
    p.add_argument("--description")
    p.add_argument("--facts")
    p.add_argument("--rationale")
    p.set_defaults(func=issue_update_body)

    p = sub.add_parser("issue-set-difficulty")
    p.add_argument("--issue-key", required=True)
    p.add_argument("--difficulty", required=True, help="抽象困难程度，正整数；omit 语义不支持")
    p.add_argument("--difficulty-reason", help='JSON 数组；省略则保留原解释')
    p.set_defaults(func=issue_set_difficulty)

    p = sub.add_parser("issue-set-assignment")
    p.add_argument("--issue-key", required=True)
    p.add_argument("--profile-id", help="Dev 执行配置 id；省略或传空表示清除分配")
    p.set_defaults(func=issue_set_assignment)
    ...
    p.add_argument("--content", required=True)
    p.add_argument("--source-discussion-ids", default="[]", help="JSON 数组；结论来源的讨论 id，如 [12,15]")
    p.add_argument("--metadata", default="{}", help="JSON 对象；只记录业务事实，不改变权限或控制流，如 {}")
