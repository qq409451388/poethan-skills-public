...
class CodeInspectorInstallerTest(unittest.TestCase):
    ...
    def test_install_is_idempotent_and_workflow_is_enforced(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("固定工具：`cictl-dev`", codex_skill_text)
            self.assertIn("固定工具：`cictl-insp`", codex_skill_text)
            self.assertNotIn(str(codex_skill / "tools" / "reviewctl-codex-dev.py"), codex_skill_text)
            self.assertNotIn(str(codex_skill / "tools" / "reviewctl-codex-insp.py"), codex_skill_text)
            self.assertIn("固定工具：`python ", trae_skill_text)
            self.assertIn("固定工具：`python ", dsh_skill_text)
            ...
            self.assertIn("cictl-insp fast pass|fail <issue_key> --content <result_summary>", codex_skill_text)
            ...
            self.assertIn("专用 `cictl-insp fast pass|fail`", source_skill_text)
            self.assertIn("当前角色命令", source_skill_text)
            self.assertIn("当前角色命令", codex_skill_text)
            self.assertIn(
                "普通 Action 不读取完整 `workflow.yaml` 或 `tool-contracts.yaml`",
                codex_skill_text,
            )
            self.assertIn("普通 Action", codex_skill_text)
            self.assertIn("cictl-dev activity show", codex_skill_text)
            self.assertIn("cictl-insp activity show", codex_skill_text)
            ...
            self.assertIn('prefix_rule(pattern=["cictl-dev"], decision="allow"', codex_rules_text)
            self.assertIn('prefix_rule(pattern=["cictl-insp"], decision="allow"', codex_rules_text)
    ...
    def test_webtool_runtime_observability_csrf_and_debug(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                ...
                self.assertLess(runtime_html.index("issue-context-get"), runtime_html.index("activity-get"))
