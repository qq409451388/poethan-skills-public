...
class CodeInspectorInstallerTest(unittest.TestCase):
    ...
    def test_install_is_idempotent_and_workflow_is_enforced(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("固定工具：`reviewctl`", codex_skill_text)
            ...
            self.assertIn("固定工具：`reviewctl`", trae_skill_text)
            self.assertIn("固定工具：`reviewctl`", dsh_skill_text)
            self.assertNotIn(str(trae_skill / "tools" / "reviewctl-trae-inspector.py"), trae_skill_text)
            self.assertNotIn(str(dsh_skill / "tools" / "reviewctl-dsh-developer.py"), dsh_skill_text)
            self.assertNotIn(str(dsh_skill / "tools" / "reviewctl-dsh-inspector.py"), dsh_skill_text)
